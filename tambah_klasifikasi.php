    ...
    if(empty($_SESSION['admin'])){
        ...
        if($_SESSION['admin'] != 1 AND $_SESSION['admin'] != 2){
            ...
            if(isset($_REQUEST['submit'])){
                ...
                if($_REQUEST['unit'] == "" || $_REQUEST['nama'] == "" || $_REQUEST['uraian'] == ""){
                    ...
                    if(!preg_match("/^[a-zA-Z0-9. ]*$/", $unit)){
                        $_SESSION['unit'] = 'Form unit hanya boleh mengandung karakter huruf, angka, spasi dan titik(.)';
                        echo '<script language="javascript">window.history.back();</script>';
                    } else {
                        ...
                        if(!preg_match("/^[a-zA-Z0-9.,\/ -]*$/", $nama)){
                            ...
                            if(!preg_match("/^[a-zA-Z0-9.,()\/\r\n -]*$/", $uraian)){
                                ...
                                $cek = mysqli_query($config, "SELECT * FROM tbl_klasifikasi WHERE unit='$unit'");
                                $result = mysqli_num_rows($cek);

                                if($result > 0){
                                    $_SESSION['duplikasi'] = 'unit sudah ada, pilih yang lainnya!';
                                    echo '<script language="javascript">window.history.back();</script>';
                                } else {

                                    $query = mysqli_query($config, "INSERT INTO tbl_klasifikasi(unit,nama,uraian,id_user) VALUES('$unit','$nama','$uraian','$id_user')");
                    ...
                    <form class="col s12" method="post" action="?page=ref&act=add">
                        ...
                        <div class="row">
                            <div class="input-field col s3">
                                ...
                                <input id="kd" type="text" class="validate" maxlength="30" name="unit" required>
                                    <?php
